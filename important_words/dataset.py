...
class Dataset(torch.utils.data.Dataset):
    def __init__(self, args, tokenizer, split_type, download=False):
        ...
        if args.UD:
            sentences = self.read_UD(
                "datasets/en_partut-ud-" + self.split_type + ".conllu"
            )
    ...
    def __getitem__(self, index):
        ...
        this_inst = {
            ...
            "sentence": {"input_id": [], "input_mask": []},
            "subtoken_map": example["subtoken_map"],
            "tokenized_sentence": example["tokenized_sentence"],
        }

        input_id, input_mask, labels = self.build_input(
            example["tokenized_sentence"], example["labels"]
        )
