...
class Display():
    def __init__(self, minimum_score_to_display=0.5, divisions = 16, categories = ["","NOUN_","PROPN_","NER_"], predict_mode = False):
    ...
    def color_label_dict(self):
        ...
        for i in np.linspace(0,self.maxscore, self.divisions):
            ...
            i = int(round(i, 3)*100)
            dic[self.categories[int(name)]+str((i))] = self.rgb_to_hex((colorval,167,125))
            if not self.predict_mode:
                dic["XX"+self.categories[int(name)]+str(i)] = self.rgb_to_hex((colorval,colorval,160))
    ...
    def get_spans(self, tokenizer, tokens, subtoken_map, predictions, scores, labels):
        ...
        for (start, end) in subtoken_map:
            token = " "+tokenizer.convert_tokens_to_string(tokens[start+1:end+1])
            ent = {
                "start":start_char,
                "end":start_char+len(token),
                "label":None
            }
            if end-start!=0:
