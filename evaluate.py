...
def run_batch_selection_eval(args, model, batch, tokenizer, display):
    ...
    total_correct = torch.sum(
        (prediction == batch["labels"])[0][:total], dtype=torch.float
    )
...
def evaluate(args, eval_dataset, model, tokenizer, desc="", epoch=0):
    ...
    for batch in epoch_iterator:
        with torch.no_grad():
            loss, total_correct, total = run_batch_selection_eval(
                args, model, batch, tokenizer, display
            )
            accuracy_dic["total"] += total
            accuracy_dic["R1"] += total_correct
            eval_loss += loss
            epoch_iterator.set_postfix(
                Accuracy=(accuracy_dic["R1"] / accuracy_dic["total"])
            )
